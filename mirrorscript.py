...
def ping(hostname):
    p = subprocess.Popen(
        ["ping", "-c 3", hostname], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    ).communicate()
    ...
    if not p[0].strip():
        ...
        response = ask(
            "\t    I got stuck at finding mirror latency. Do you want to retry[y] or skip[n]?",
            "n",
        )
        if response:
            ping(hostname)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="automatically select the best kali mirror server and apply the configuration"
    )
    parser.add_argument(
        "-v", "--verbose", help="enable verbose output", action="store_true"
    )
    parser.add_argument(
        "-https", help="use HTTPS in apt transport (default HTTP)", action="store_true"
    )
    parser.add_argument(
        "-src", help="enable sources packages (default disable)", action="store_true"
    )
    ...
    # Banner
    print("#")
    print("# kali linux mirror update tool")
    ...
    for hostname in new_urls:
        ...
        while True:
            ...
            try:
                ...
                if not ask(
                    "\t[!] Something went wrong. would you like to try again [y] or [n].",
                    "y",
                ):
                    print("\t    Exiting ...\n")
                    sys.exit(1)
    ...
    for line in file.readlines():
        if (re.search(r"^deb http(?:s|)://http.kali.org/kali", line, re.I)) or (
            re.search(r"^deb-src http(?:s|)://http.kali.org/kali", line, re.I)
        ):
            newline = "#" + line
            file.write(newline)
            contents.append(newline)
        elif re.search(r"^# Autogenerated script ", line, re.I):
    ...
    temp = "sh -c 'echo \"\n# Autogenerated script\" >> /etc/apt/sources.list'"
